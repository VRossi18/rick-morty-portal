...
export function EpisodesPage() {
      ...
      const loadData = async () => {
         ...
         try {
            if (selectedCharacterIds.length > 0) {
               const all = await fetchAllEpisodes(listFilters);
               const filtered = all.filter((ep) =>
                  episodeIncludesAnyCharacter(ep, selectedCharacterIds),
               );
               const paged = paginateEpisodes(filtered, page);
   ...
   }, [page, listFilters, selectedCharacterIds, t]);
      ...
      >
         ...
         <main className="mx-auto max-w-[1400px] px-6 pb-20">
            ...
            <div className="relative min-h-[20rem]">
               {error ? (
                  ...
                  <div
                     className={`columns-1 gap-6 sm:columns-2 xl:columns-3 ${loading ? 'pointer-events-none opacity-45' : ''}`}
                  >
                     {episodes.map((ep) => {
                        const interaction =
         ...
         <footer className="flex items-center justify-center gap-4 pb-8">
            ...
            <span className="text-sm font-semibold text-[var(--text-color)]">
               {t('episodes.pagination.pageOf', { current: page, total: pageInfo?.pages ?? 1 })}
            </span>
